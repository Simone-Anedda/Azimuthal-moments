...
#include "FCN.h"
    ...
    Belle12_A12UL_z1z2, Belle12_A12UL_z1z2,\
    ...
    extern TRANSVX::getTRANSV_x myTransv;
    extern COLZ::getCOL_z myCol;
    ...
    const double pi = atan(1), Mp = 0.9383, Mpi = 0.1396, EulerConst = exp(1.); //Mp=proton mass, Mpi=charged pion mass (all measured in GeV)
...
//     extern double asQ0, Q0alphas, muR_Q, Q0pdf;
    extern int Npoints; //nloop, 
    // iset_ff, Npoints;
    
    bool evo_flag, poly_flag = true;
//     bool hoppet_transv, hoppet_col;
    extern bool diffsame_flag, fixed_Q2, dssv_init, grv_flag; 
    
    extern bool hessian_flag, MC_flag;  
    
    extern vector<double> the_chi2s;//, weights;
    
//     int iweights = 0;
//     ofstream transv_pars("./transv_pars.dat");
...
//     cout<<endl;
    ...
    myTransv.eval(x, Q02, myTransv.transv_model, myTransv.transv_evo, evo_par);
...
//     cout<<"Q02 pip = "<<Q02<<endl;
    ...
    myCol.eval(z, Q02, charge, myCol.col_model, myCol.col_evo, FF, evo_par);
...
void Collins_PT(const double & av_z, const double & av_Q2, const std::vector<double> & par, const string & dataname){
    ...
    int model = myCol.col_model; 
    int widths = myCol.col_widths; //choice of (<k^2_\perp>,<p^2_\perp>)
    std::size_t found = dataname.find("comp");        
    
    double sqMC;
    
    //COLLINS 2015 fit
    if(model == 0 || model == 1) sqMC = par[8];
    if(model == 100) sqMC = par[9];
    ...
    if(model == 0 || model == 1 || model == 100) pperpC_width = (sqMC * p_width) / (sqMC + p_width); //cout<<"pperpC="<<pperpC_width<<endl; fit 2015 - MC^2 is the parameter
...
//     cout<<"In integrand: int_had = "<<int_hadron<<"\t int_ch = "<<int_charge<<endl;
    ...
    if(!fixed_Q2){
        ...
        if(myCol.col_evo == 0){ 
            ...
            myCol.eval(z1, int_av_Q2, int_charge, myCol.col_model, myCol.col_evo, FF_ppz1, int_par);
            for(int i = 0; i < COL_ppz1.size(); i++) COL_ppz1[i] = myCol.COL_z[i];
            
            myCol.eval(z2, int_av_Q2, int_charge, myCol.col_model, myCol.col_evo, FF_ppz2, int_par);
            for(int i = 0; i < COL_ppz2.size(); i++) COL_ppz2[i] = myCol.COL_z[i];
            
            int_charge *= -1; //to call pi- Collins
                
            myCol.eval(z1, int_av_Q2, int_charge, myCol.col_model, myCol.col_evo, FF_pmz1, int_par);
            for(int i = 0; i < COL_pmz1.size(); i++) COL_pmz1[i] = myCol.COL_z[i];
            
            myCol.eval(z2, int_av_Q2, int_charge, myCol.col_model, myCol.col_evo, FF_pmz2, int_par);
...
//             COL_pmz2 = CollinsHoppetEval(z2, int_charge, f);
    ...
    if(fixed_Q2){
        ...
        if(myCol.col_evo == 0){
            //call for the Collins functions
            myCol.eval(z1, FIXEDQ2, int_charge, myCol.col_model, myCol.col_evo, FF_ppz1_fixedQ2, int_par);
            for(int i = 0; i < COL_ppz1.size(); i++) COL_ppz1[i] = myCol.COL_z[i];
            
            myCol.eval(z2, FIXEDQ2, int_charge, myCol.col_model, myCol.col_evo, FF_ppz2_fixedQ2, int_par);
            for(int i = 0; i < COL_ppz2.size(); i++) COL_ppz2[i] = myCol.COL_z[i];
            
            int_charge *= -1; //to call pi- Collins
                
            myCol.eval(z1, FIXEDQ2, int_charge, myCol.col_model, myCol.col_evo, FF_pmz1_fixedQ2, int_par);
            for(int i = 0; i < COL_pmz1.size(); i++) COL_pmz1[i] = myCol.COL_z[i];
            
            myCol.eval(z2, FIXEDQ2, int_charge, myCol.col_model, myCol.col_evo, FF_pmz2_fixedQ2, int_par);
            for(int i = 0; i < COL_pmz2.size(); i++) COL_pmz2[i] = myCol.COL_z[i];
...
//             COL_pmz2 = CollinsHoppetEval(z2, int_charge, f);
    ...
    if(myCol.col_model == 2 || myCol.col_model == 3) numFact *= z1 * z2;
...
void SetTarget(const int target_in, const int evo_in){
    
    int target = target_in, evo = evo_in;
...
// double Collins_Asy(const std::vector<double> &bin, const int hadron_in, const int charge_in, const int target_in, const int process_in, const std::vector<double> &par, const string dataname_in, const int epem_sign_in){
double Collins_Asy(const std::vector<double> &bin, const int hadron_in, const int charge_in, const int target_in, const string process_in, const std::vector<double> &par, const string dataname_in, const string epem_sign_in){
    ...
    int hadron = hadron_in, charge = charge_in, target = target_in;// process = process_in, epem_sign = epem_sign_in;
    std::string process(process_in), epem_sign(epem_sign_in);
    int model = myCol.col_model, evo = myCol.col_evo; //model and evo for Collins z-dependence
    int order;
    ...
    // if(iset_ff == 1) order = 0;
    // if(iset_ff == 2) order = 1;
    // int_order = order;
...
//     if(process == 0) cout<<"In Collins asy: av_z = "<<av_z<<endl;
//     if(process > 0) cout<<"In Collins asy: av_z1 = "<<av_z1<<"\tav_z2 = "<<av_z2<<endl;
...
//     if(process == 0){ //SIDIS
    if(process == "SIDIS"){ //SIDIS
        ...
        // if(!grv_flag) pdf->xfxQ2(av_x, av_Q2, PDF); //call lhapdf PDF set
        ...
        // myFF.FF_eval("DSS", iset_ff, hadron, charge, order, av_z, av_Q2);
...
//             int_av_Q2 = FIXEDQ2;
        ...
        //Transversity
        if(myTransv.transv_evo == 0){
            
            myTransv.eval(av_x, av_Q2, model, evo, par);
        ...
        else if(myTransv.transv_evo == 1){ //TO DO: DEVELOP HOPPET EVOLUTION FOR COLLINS
...
//             cout<<"in FCN.cpp: "<<h1[7]<<"\t"<<h1[8]<<endl;
        ...
        if(myCol.col_evo == 0){
            ...
            myCol.eval(av_z, av_Q2, charge, model, evo, FF, par);
            for(int i = 0; i < COL_z.size(); i++) COL_z[i] = myCol.COL_z[i];
//             for(int i=0; i<COL_z.size(); i++) cout<<"asy: COL_z["<<i<<"]="<<COL_z[i]<<endl;
        
        }
        
        else if(myCol.col_evo == 1){ //TO DO: CHECK HOPPET EVOLUTION FOR COLLINS
...
//             for(int i=0; i<COL_z.size(); i++) cout<<"asy: COL_z["<<i<<"]="<<COL_z[i]<<endl;
        ...
        SetTarget(target, myTransv.transv_evo);
...
//         cout<<"before F_UT = "<<F_UT<<endl;
        ...
        if(model == 0 || model == 1 || model == 10 || model == 100){ //fit 2015
...
//             cout<<"after F_UT = "<<F_UT<<endl;
        ...
        if(model == 2 || model == 3){ //fit 2018 - REWEIGHTING - unbiased
        ...
        if(found!=std::string::npos) output *= - 1 / D_NN; //COMPASS \pi mismatch (they measure sin(\phi_h+\phi_S +- \pi) that is -sin(\phi_h+\phi_S)) and divide out the depolarisation factor        
        
//         if(found!=std::string::npos) output *= - 1; //COMPASS \pi mismatch (they measure sin(\phi_h+\phi_S +- \pi) that is -sin(\phi_h+\phi_S)) 
...
//         for( comp = 0; comp < NCOMP; comp++ ) cout<<"CUHRE RESULT:\t"<<integral[comp]<<" +- "<<error[comp]<<"\tp = "<<prob[comp]<<endl;
        ...
        if(model == 0 || model == 1 || model == 10 || model == 100) output *= (EulerConst / 2) * ssth * (pow(av_pt, 2) / (sqMC + p_width)); //z1-z2-integrated asymmetries - fit2015
        if(model == 2 || model == 3) output *= 4.0 * pow(Mpi, 2) * pow(av_pt, 2) * ssth * (p_width / pow(pperpC_width, 3)); //z1-z2-integrated asymmetries - fit2018 - REWEIGHTING - unbiased
        
//         cout<<"A0_PT = "<<output<<endl;
    }
    
    
//     if(process == 2){ //e+e-(z1z2)
    if(process == "e+e-(z1z2)"){ //e+e-(z1z2)
        ...
        if(myCol.col_evo == 0){ 
            ...
            myCol.eval(av_z1, av_Q2, charge, model, evo, FF_ppz1, par);
            for(int i = 0; i < COL_ppz1.size(); i++) COL_ppz1[i] = myCol.COL_z[i];
            myCol.eval(av_z2, av_Q2, charge, model, evo, FF_ppz2, par);
            for(int i = 0; i < COL_ppz2.size(); i++) COL_ppz2[i] = myCol.COL_z[i];
            
            charge *= -1; //to call pi- Collins
            
            myCol.eval(av_z1, av_Q2, charge, model, evo, FF_pmz1, par);
            for(int i = 0; i < COL_pmz1.size(); i++) COL_pmz1[i] = myCol.COL_z[i];
            myCol.eval(av_z2, av_Q2, charge, model, evo, FF_pmz2, par);
            for(int i = 0; i < COL_pmz2.size(); i++) COL_pmz2[i] = myCol.COL_z[i];
        }
...
//         if(epem_sign == 1){ //A0_UL
        if(epem_sign == "A0UL"){ //A0_UL
            
            output = (numU / denU) - (numL / denL);
        }
        
//         if(epem_sign == 2){ //A0_UC
        if(epem_sign == "A0UC"){ //A0_UC
        ...
        if(model == 0 || model == 1 || model == 10 || model == 100){ 
            //Asymmetry expressions taken from  arXiv:1809.09500v1 [hep-ph]
            Collins_PT(av_z1, av_Q2, par, dataname); //note: av_z1 is not used here, just needed to call the function to fix FC_ee factor
         
            output *= (1./4.) * ssth * ((av_z1 * av_z2) / (pow(av_z1, 2) + pow(av_z2, 2))) * FC_ee; //PT1-integrated asymmetries
        }
        
        if(model == 2 || model == 3) output *= (4.0 * pow(Mpi, 2) / pperpC_width) * ssth * (pow(av_z1 * av_z2, 2) / (pow(av_z1, 2) + pow(av_z2, 2))); //PT1-integrated asymmetries
...
//     if(data.process == 1){ //e+e- PT (to get parameters for the numerical integration)
    ...
    if(evo_flag){
        ...
        if(myTransv.transv_evo == 1) hoppetEvolve(asQ0, Q0alphas, nloop, muR_Q, transv_subroutine, Q0pdf);
...
//                     cout<<MC_flag<<"\t"<<data.theory[i]<<"\t"<<data.theory_0[i]<<endl;
                }
                
//                 if(abs(data.theory[i]) <= 1.0 && MC_flag){
//                     
...
// //                     cout<<"theory_0["<<i<<"] = "<<data.theory_0[i]<<"\t"<<"theory["<<i<<"] = "<<data.theory[i]<<endl;
// //                     cout<<pow(data.theory[i] - data.theory_0[i], 2)<<"\t"<<weights[iweights]<<endl;
//                     if(iweights < weights.size()){
// 
//                         data.theory_sigma[i] += weights[iweights] * pow(data.theory[i] - data.theory_0[i], 2);
// //                         data.theory_min[i] += weights[iweights] * pow(data.theory[i] - data.theory_0[i], 2);
// //                         if(data.fname == Herm10_pip_x.fname) cout<<"before: "<<iweights<<"\t"<<data.theory_sigma[i]<<endl;
//                     }
//                     
//                     if(iweights == weights.size()){
//                         
//                         data.theory_sigma[i] = sqrt(data.theory_sigma[i]);
// //                         data.theory_min[i] = sqrt(data.theory_min[i]);
// //                         if(data.fname == PercheHerm10_pip_x.fname) cout<<"after: "<<iweights<<"\t"<<data.theory_sigma[i]<<endl;
//                         
//                     }
//                 }
            }
...
double FCN::operator()(const std::vector<double>& par) const{
    ...
    if(myTransv.transv_evo == 1 || myCol.col_evo == 1)
    {
        evo_flag = true;
...
//     if(poly_flag){
    ...
    output += Collins_Asy_loop(Herm20_pip, par);
    output += Collins_Asy_loop(Herm20_pim, par);
...
//     cout<<"beta_u = "<<par[3]<<"  beta_d = "<<par[4];
    ...
    if(myCol.col_evo == 0 && myTransv.transv_evo == 0) cout<<"\tcount="<<icount<<endl;
    if(myCol.col_evo == 1 || myTransv.transv_evo == 1) cout<<"\tevo_count="<<icount<<endl;
    else cout<<endl;
