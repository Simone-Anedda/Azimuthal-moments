...
BINS = [
    {"label": r"$0.1 < z_1 < 0.2$", "file": "kT_max_50_kT_min_3_Vs_13000_thetac_0.03_z1_0.1_0.2_U_L.txt"},
    {"label": r"$0.2 < z_1 < 0.3$", "file": "kT_max_50_kT_min_3_Vs_13000_thetac_0.03_z1_0.2_0.3_U_L.txt"},
    {"label": r"$0.3 < z_1 < 0.4$", "file": "kT_max_50_kT_min_3_Vs_13000_thetac_0.03_z1_0.3_0.4_U_L.txt"},
    {"label": r"$0.4 < z_1 < 0.5$", "file": "kT_max_50_kT_min_3_Vs_13000_thetac_0.03_z1_0.4_0.5_U_L.txt"},
    {"label": r"$0.5 < z_1 < 0.7$", "file": "kT_max_50_kT_min_3_Vs_13000_thetac_0.03_z1_0.5_0.7_U_L.txt"},
    {"label": r"$0.7 < z_1 < 0.9$", "file": "kT_max_50_kT_min_3_Vs_13000_thetac_0.03_z1_0.7_0.9_U_L.txt"},
]
...
XLIM = (0.05, 0.85)
YLIM = (-0.08, 0.08)   # None per automatico
...
def make_figure(bins, n_rows, n_cols):
    ...
    for i, bin_cfg in enumerate(bins):
        ...
        # --- Dati ---
        try:
            z2, y_u, y_l = load_data(bin_cfg["file"], COL_Z2, COL_Y, COL_Y2)
            lu, = ax.plot(z2, y_u, lw=LINE_WIDTH, ls=LINE_STYLE,   color=COLOR_U)
            ll, = ax.plot(z2, y_l, lw=LINE_WIDTH, ls=LINE_STYLE_L, color=COLOR_L)
            if i == 0:
        ...
        if i == 2:
            x_seg = [0.1, 0.25]
            ax.plot(x_seg, [Y_U] * 2, lw=2, ls=LINE_STYLE,   color="black")
            ax.plot(x_seg, [Y_L] * 2, lw=2, ls=LINE_STYLE_L, color="black")
            ax.text(0.3, Y_U_LABEL, r"$U$", size=15)
    ...
    if legend_handles:
        fig.legend(legend_handles, [r"$U$", r"$L$"],
...
if __name__ == "__main__":
    ...
    plt.savefig("ratio_U_and_L.pdf", dpi=150, bbox_inches="tight")
    plt.savefig("ratio_U_and_L.png", dpi=150, bbox_inches="tight")
    #plt.show()
    print("Salvato: ratio_U_and_L.pdf / ratio_U_and_L.png")
